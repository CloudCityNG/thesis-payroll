...
class permission extends crackerjack_model{
	...
	public function getParentModule($id = false,$label =false){
		...
		foreach ($query as $get) {
			...
			$module[$get->xparentlabel_id]['module'][$get->xparentmodule_id]['xmodule'] = $get->parentmodule;
	...
	public function subModule($submoduleId = false,$moduleId =false){
		$return = array();
			$result = $this->crud->read('SELECT * FROM _xmodule AS xm INNER JOIN _xparentmodule AS pm ON xm.xparentmodule_id = pm.xparentmodule_id WHERE xm.status=1 AND pm.status=1',array(),'obj');
			$submodule = array();
			$module = array();
			foreach ($result as $get) {
				$submodule[$get->xparentmodule_id][$get->xmodule_id]['submodule'] = $get->module;
				$submodule[$get->xparentmodule_id][$get->xmodule_id]['_create'] = $get->_xcreate;
				$submodule[$get->xparentmodule_id][$get->xmodule_id]['_read'] = $get->_xread;
				$submodule[$get->xparentmodule_id][$get->xmodule_id]['_update'] = $get->_xupdate;
				$submodule[$get->xparentmodule_id][$get->xmodule_id]['_delete'] = $get->_xdelete;
				$submodule[$get->xparentmodule_id][$get->xmodule_id]['_export'] = $get->_xexport;
				$submodule[$get->xparentmodule_id][$get->xmodule_id]['_print'] = $get->_xprint;
				$submodule[$get->xparentmodule_id][$get->xmodule_id]['_url'] = $get->_xurl;
			}
			$return = $submodule;
	...
	public function getUserPermission($id){
		...
		switch ($id) {
				case 1:
				$query = "SELECT _xurl,parentmodule,_xcreate,_xread,_xupdate,_xdelete,_xprint,_xexport,xparentlabel_id,xparentmodule_id FROM _xparentmodule WHERE status = 1 ORDER BY xparentlabel_id,xparentmodule_id ASC"; 
				break;
				default:
						$query = "SELECT pm._xurl,pm.parentmodule,a._xcreate,a._xread,a._xupdate,a._xdelete,a._xprint,a._xexport,a.xparentmodule_id,pm.xparentlabel_id FROM _xacl AS a INNER JOIN _xparentmodule AS pm ON a.xparentmodule_id = pm.xparentmodule_id WHERE a.xroles_id=:rid AND a._xread = 1 AND pm.status = 1 AND pm.xparentmodule_id != 1 ORDER BY  pm.xparentlabel_id";			
				break;
			}
